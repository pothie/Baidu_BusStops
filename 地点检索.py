...
ci = ""
ar = ""
tag = "" #百度定义标签 http://lbsyun.baidu.com/index.php?title=lbscloud/poitags
#left = "41.716777,123.227841" #矩形检索区域左下角坐标
#right = "41.890786,123.623671" #矩形检索区域右上角坐标
...
def window():
    ...
    Lleft = tk.Label(frame1,text="搜索区左下角经纬度坐标").grid(row=0)
    left= tk.Entry(frame1)
    left.grid(row =0,column=1)
    Rleft = tk.Label(frame1,text="纬度在前，经度在后，用半角逗号隔开").grid(row=0,column=2)
    left.focus()
	
    Lright = tk.Label(frame1,text="搜索区右上角经纬度坐标").grid(row=1)
    right=tk.Entry(frame1)
    right.grid(row=1,column=1)
    Rright = tk.Label(frame1,text="纬度在前，经度在后，用半角逗号隔开").grid(row=1,column=2)

			
    Ltag = tk.Label(frame1,text="搜索关键字").grid(row=2)
    ...
    path = StringVar()		
    Label(frame1,text = "目标路径:").grid(row = 4, column = 0)
    Entry(frame1, textvariable = path).grid(row = 4, column = 1)
    Button(frame1, text = "路径选择", command = selectPath).grid(row = 4, column = 2)
    ...
    b1=tk.Button(frame1,text='搜索生成excel',command = setVarE).grid(row=5,column=1)
    #b2=tk.Button(frame1,text='搜索生成txt',command = setVarT).grid(row=5,column=1)
	
    window.mainloop()
		
def request_data_txt(bLeft,uRight):
    ...
    try:
        ...
        tk.messagebox.showinfo("ERROR","百度每日POI访问量限制")
        os._exit(0)
	
    f = open(Path+tag+'.txt','a') 
   
    for i in range(total_page_num):
        params['page_num'] = i
        request = requests.get(url,params=params)
        
        time.sleep(1) #百度并发量限制，个人认证或企业认证可提高5倍-10倍，可以是小数
        for item in json.loads(request.text)['results']:
            name = item['name']             
            lat = item['location']['lat']
            lng = item['location']['lng']
            f.write(name+','+str(lng)+','+str(lat)+','+'\n')        
    ...
    if total == 400:
        ...
        for i in range(total_page_num):
            ...
            try:
		        ...
		        #()中可以是小数
                for item in json.loads(request.text)['results']:
                    if (ar == (item['area'] or None) or item['area'] == None) and (pro == (item['province'] or None) or item['area'] == None) and (ci == (item['city'] or None) or item['area'] == None):
                        name = item['name']             
                        lat = item['location']['lat']
                        lng = item['location']['lng']              
